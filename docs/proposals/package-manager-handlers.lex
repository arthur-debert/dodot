...
6. Availability And Bootstrapping
    ...
    6.1. Availability Is Resolved By Path, Not By PATH
        ...
        A second reason holds even for a shim invoked with nothing repo-local in reach, and it is the one that matters for [#6.2]: probing tests whether a *file* is there. For a concrete binary, presence means the manager is installed, which is exactly the question the probe is asking. For a shim, presence means only that the version manager is installed — the runtime behind it may not be, and the shim will exist either way. The three outcomes then stop being decidable at probe time: dodot would report a manager as present and discover at apply time that it is not, converting a clean skip into an error.
...
12. Open Questions
    ...
    - How dodot should find an npm supplied by a version manager. The fixed candidate list in [#6.1] cannot reach either kind: nvm and fnm put the selected Node version in the path, and volta, asdf, and mise answer through shims, which the same section rules out as candidates. Four alternatives are on the table — a controlled PATH lookup for this one manager, a root-only trusted executable path, resolving a shim to the concrete binary behind it, or admitting shims as candidates and controlling the environment they resolve in — and each gives back something this document argues for elsewhere, so the choice needs making on its own terms rather than as a detail of this proposal. The last one is the cheapest to describe and the most demanding to get right: it would have to establish not just what a shim reads, but how a probe distinguishes an installed runtime from a shim that merely exists.
