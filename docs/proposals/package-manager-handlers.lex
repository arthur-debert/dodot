...
6. Availability And Bootstrapping
    ...
    6.1. Availability Is Resolved By Path, Not By PATH
        ...
        This is more robust and it is also safer. A PATH lookup for `cargo` can resolve to a rustup shim which, invoked from a directory containing a `rust-toolchain.toml`, will silently download an entire toolchain before answering — observed during research. Executing a known path avoids that.

        What does the avoiding is the *concreteness* of the binary, not the absoluteness of the path, and the distinction matters enough to state as a rule on the candidate list: a candidate must be a real executable, never a version-selecting shim. An absolute path to a shim is still a shim — it reads the version manager's configuration, discovered by walking up from the working directory dodot inherited, and selects or downloads a runtime accordingly. That is the rustup behaviour this section rejects, arriving through the front door. A candidate list containing shims would also undercut the reason executables stay out of pack-controlled configuration, since the shim's answer is itself steered by a file the repo can contain. See [#9.1].

        Candidate lists must include the Homebrew prefixes, not only each manager's native location, because brew-installed `node` and `uv` are the common macOS case. For `uv`: `~/.local/bin/uv`, `/opt/homebrew/bin/uv`, `/usr/local/bin/uv`, `/home/linuxbrew/.linuxbrew/bin/uv`. For `npm`: `/opt/homebrew/bin/npm`, `/usr/local/bin/npm`, `/home/linuxbrew/.linuxbrew/bin/npm`.
        ...
        npm is the one manager where a fixed list is knowingly incomplete, and the gap is worth stating rather than discovering later. Every way of getting npm from a version manager is out of reach of the rule above: nvm and fnm expose it from a path containing the selected Node version, so there is nothing to enumerate at compile time, and volta, asdf, and mise do publish stable paths but publish them as exactly the shims a candidate may not be. Combined with the absent-manager outcome in [#6.2], a user whose only npm comes from any of them gets a silent permanent skip of their `npm.packages.txt`.

        Two things follow. The absent-manager line must report the locations that were probed, so "npm is not installed" and "npm is installed somewhere dodot does not look" are distinguishable ([#6.2]). And the remaining question is left open rather than settled in passing, because every candidate answer costs something this proposal has already refused elsewhere: a controlled PATH lookup reintroduces the resolution this section avoids, a root-only trusted executable path has to survive the reasoning that keeps executables out of user configuration in [#9.1], and resolving a shim to the concrete binary behind it means dodot deciding which runtime the user meant. See [#12].

        One narrower version of the same exposure survives the shim rule and is worth naming, because a concrete binary is necessary and not sufficient. dodot runs provisioning commands with the working directory it inherited from whoever invoked `dodot up`, and npm reads `.npmrc` by walking up from that directory — so a real `/opt/homebrew/bin/npm` invoked from inside a project can still pick up that project's registry or prefix. Manager commands therefore need a controlled working directory rather than an inherited one. That is shipped behaviour rather than something this proposal introduces; see [#11.10].
...
11. Defects To Fix Alongside
    ...
    11.10. Provisioning commands inherit the invoking shell's working directory

        `ShellCommandRunner::run` spawns with `Command::new(executable).args(arguments)` and never sets `current_dir`, so every provisioning command runs wherever the user happened to be when they typed `dodot up`. Managers read per-directory configuration from there — npm walks up for `.npmrc`, and it is the same mechanism that makes a version-manager shim answer differently per directory ([#6.1]). The result is a command whose behaviour depends on the user's shell history rather than on the pack. These commands should run from a controlled directory.
...
12. Open Questions
    ...
    - How dodot should find an npm supplied by a version manager. The fixed candidate list in [#6.1] cannot reach either kind: nvm and fnm put the selected Node version in the path, and volta, asdf, and mise answer through shims, which the same section rules out as candidates. Three alternatives are on the table — a controlled PATH lookup for this one manager, a root-only trusted executable path, or resolving a shim to the binary behind it — and each gives back something this document argues for elsewhere, so the choice needs making on its own terms rather than as a detail of this proposal.
