Design Specification: Shell Hookup Ergonomics
    ...
    :: note ::
        *Status: proposed.* Epic RCS01. Successor to [./shipped/shell-hookup.lex] (INS01), which shipped the activation evidence this document extends. Read that first: the generation stamp, the heartbeat, the signal ladder, the probe, and `dodot install` are all defined there and assumed here.
        ...
        Two shipped deviations, where the code is the authority. The diagnosis probe initially landed *inside* `dodot probe shell-init`, not as the separate `dodot probe shell-hookup` command [#3.1] proposes (epic `#288`, decision 6). SIV01 then replaced its default full trace with targeted time-to-hook verification; the full diagnosis now requires `--trace-hook`, while `--no-verify` and the passive views start no shell ([./shipped/targeted-shell-init-verification.lex]). And the version-skew hint points at `$PATH` resolution generically ("check which one your PATH finds first") rather than naming both binary paths as [#2.3] promises; the explicit trace is where both paths get named.
...
3. The Hookup Diagnosis Probe
    ...
    3.1. Why This Is a Separate Command
        ...
        :: note ::
            *Superseded first by epic `#288`, decision 6, then by SIV01.* No `dodot probe shell-hookup` command exists. The diagnosis shipped folded into `dodot probe shell-init`; SIV01 later made the bare command a targeted time-to-hook verification and put hook-line diagnosis behind `--trace-hook`. `--no-verify` opts out of the targeted spawn; the passive views (a `<PACK[/FILE]>` filter, `--runs`, `--history`, `--errors-only`) never verify or trace. See [../user/commands/probe.lex] §4 and [./shipped/targeted-shell-init-verification.lex].
