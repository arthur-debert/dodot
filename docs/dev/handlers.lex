...
10. Testing
    ...
    A third pattern guards the documentation. The tests in `handlers/catalog.rs` hold `HANDLER_CATALOG` and the registry equal in both directions, and re-render [./../reference/handler-registry.lex] to compare against the checked-in file — so a handler that ships without a catalog row, or with a stale generated page, fails `pixi run test`. A third test, `contextual_rosters_list_every_handler`, reads the pages in `CONTEXTUAL_ROSTERS` — but only the roster each one marks off, not the page around it — and fails when a registered handler is missing from it. That is the drift that actually happened when `nix`, `external`, and `gate` landed. The scoping is what gives the test teeth: handler names like `path` and `gate` occur all through ordinary prose, so a page-wide search stays green even after a handler is deleted from the table it belongs in. What it checks is presence and nothing more — it cannot tell that a roster put a handler in the wrong phase, gave it the wrong priority, or described it wrongly. Its own helpers are covered too: a fixture where a handler is named only outside the markers has to read as missing, and a page that lost its markers is reported as a broken page rather than as one that dropped every handler at once.
