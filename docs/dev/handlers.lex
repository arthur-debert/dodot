...
9. Adding a New Handler
    ...
    6. Update the user-facing handler docs (add a new snippet under `docs/user/handlers/` and link it from [./../user/handlers.lex]; update the defaults table in [./../user/handlers/mappings.lex]) and the reference ([./../reference/handlers.lex]) — the phase enum is contributor-visible, but the handler itself surfaces in user docs as soon as it ships.
