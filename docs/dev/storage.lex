...
5. Sentinel Format
    ...
    Because the checksum is part of the sentinel name, any change to the input content produces a new sentinel name — which is how dodot detects that an `install.sh` has been edited or that a preprocessor produced different output on a new machine. Detection is not application: [`DataStore::did_run`] reports the mismatch as `RanDifferent`, the executor skips the command with a "ran older version" notice, and applying the edit takes `dodot up --provision-rerun`.
