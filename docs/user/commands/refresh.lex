...
6. Watch out for
    ...
    - *Mutating refresh is Safety Lock-gated.* The default and `--quiet` modes can touch source mtimes and require approval for an implicitly discovered root. `--list-paths` is read-only and never establishes trust. For non-interactive use, set `DOTFILES_ROOT` explicitly. See [./../safety-lock.md].
