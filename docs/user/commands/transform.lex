...
5. Watch out for
    ...
    - *Mutating transform actions are Safety Lock-gated.* `transform check` and `transform install-hook` require approval for an implicitly discovered root; `check --dry-run` and `transform status` stay read-only and never establish trust. For non-interactive use, set `DOTFILES_ROOT` explicitly. See [./../safety-lock.md].
