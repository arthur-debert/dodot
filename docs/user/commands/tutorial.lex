...
5. Watch out for
    ...
    - *The real deployment step is Safety Lock-gated.* If the repo the tutorial found is an implicitly discovered root you haven't approved, saying yes to "apply for real" leads to the same approval prompt a standalone `dodot up` shows — refusing it ends the tutorial with nothing deployed. Every step before it only reads. See [./../safety-lock.lex].
