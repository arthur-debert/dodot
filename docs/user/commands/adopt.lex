...
2. What it does
    ...
    Once publication succeeds, each original is replaced by a symlink to its published pack path, one source at a time:

    - *A file* — including a symlink you adopt with `--no-follow` — gets its symlink created at an adjacent temporary name and renamed over the original. That rename is atomic, so the original path holds your readable file or the finished symlink and never anything in between.
    - *A directory* is renamed to an adjacent backup, then symlinked, then the backup is removed. If the symlink fails the backup is renamed back and the directory stands at its original path with everything in it. This is recoverable rather than atomic: a `dodot adopt` killed between the two steps leaves the directory at a `.dodot-adopt-old-<name>-<id>` path beside where it was, and `mv` puts it back.

    Sources are independent, and a failure on one does not stop the run or touch the others. Every source you asked for is attempted; each one ends either replaced, or untouched with its pack entry taken back out — which means the whole of what publication did for that entry, undone. Under `--force` the destination it displaced is renamed back; otherwise the published entry goes, along with the directories publication created inside the pack for it and nothing else, and along with the pack itself when the pack is one this run created and no source was replaced at all. A source that was already replaced stays replaced. Taking that entry back out can fail in turn, and `adopt` then says so instead of deleting anything to get past it — the paragraph below is what that run looks like.

    Independence is the guarantee on offer here, and it is deliberately not a bigger one: your `$HOME` and your dotfiles repo may sit on different filesystems and so may two sources, so there is no invocation-wide atomic replacement and `adopt` does not claim one. What it claims is that each source ends in one of the two states above whatever happened to the others, and never in a state `adopt` has not told you about.

    Once every source has succeeded or been put back, the staging directory goes, and with it whatever `--force` displaced. That discard is where your `--force` takes effect. The one run that keeps the directory instead is one where putting an entry back is itself what failed — the destination `--force` displaced could not return to the pack, or the copy publication put there could not come out. It also stays put when the path no longer holds what the run published — another process replaced the entry, or edited a file inside a directory the run had just copied in. `adopt` checks a published directory against its whole contents before moving it, so a concurrent edit anywhere in that tree leaves the tree standing rather than carrying someone else's work off with it. Nothing is deleted to get past that: the report names the entry and the path its content is at, so you can move it yourself. A pack entry still standing there is a duplicate of a source that was not replaced, and removing it by hand is all that run left undone.

    *Exit status.* `0` when every source you asked for was replaced, `1` when any of them failed — so `dodot adopt && …` stops on a partial adoption instead of continuing. The result still lists every source either way, replaced and failed alike, so you can see what did land. Entries `adopt` left in place (§5) don't move it: those were never planned for adoption. A refusal — anything in §5, a destination conflict, a cross-pack collision — exits on its own error status without having written anything.
...
8. Watch out for
    ...
    - *A partially adopted run exits `1` and is not undone.* If one source of several can't be replaced, the others stay adopted — that's the point of treating sources independently — and only the failed one is put back. The nonzero status is how a script tells that run from a complete one; the report names each failed source and why. Re-run `adopt` on the failed paths once you've fixed what stopped them.
