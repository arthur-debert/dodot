...
7. Watch out for
    ...
    - *An `externals.toml` that's still a template stops the run.* Before writing anything, `adopt` checks that no other pack already claims the paths you're about to deploy to. A pack's `externals.toml` declares its targets inside the file, so `adopt` has to read it — and if that file only exists as `externals.toml.tmpl` and you have never run `dodot up` on that pack, there is nothing to read. Rendering it here would resolve its secrets and write its output for a run you haven't agreed to yet, so `adopt` refuses instead and names the file. Run `dodot up` for that pack once, then re-run `adopt`. `--force` doesn't skip this: it overrides what dodot found in the way, not what dodot hasn't looked at.
