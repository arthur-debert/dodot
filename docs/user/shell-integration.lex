...
3. What the init script contains
    ...
    On any host with Homebrew installed — macOS or Linux — the script *opens* with Homebrew's environment — `brew shellenv` output captured verbatim at the last `dodot up` and cached in the datastore — so brew's `$PATH` entries exist before any pack script runs and your rc needs no bootstrap line of its own. It is emitted first so that dodot's own PATH additions below it win. On macOS this block is the one command dodot puts on the shell startup path — brew's own `path_helper` re-exec — while a Linux capture is plain `export` lines that run nothing; [./configuration.lex] §10 documents what each costs, how the capture is cached, and the `[shell] homebrew = "auto" | "off"` key that controls it.
