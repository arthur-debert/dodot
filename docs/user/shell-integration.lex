...
3. What the init script contains
    ...
    A flat shell script: one `source <path>` line per shell-handler source, and a *single* composed `export PATH="<dir1>:<dir2>:...:$PATH"` line covering every pack's path-handler directory — computed once per `dodot up`, not rebuilt by runtime prepends — with one `# [<pack>]` comment per contributing pack above that line so you can tell what came from where. Which pack's directory ends up where in that composed line, what wins on a collision, and how a raw `export PATH=` inside a pack's own shell script survives into it, is the precedence contract in [./handlers/path.lex] §3.
