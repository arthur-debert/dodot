...
7. Shell integration issues
    ...
    7.1. "Aliases / PATH additions from a pack don't take effect"
        ...
        - Shell hookup: your shells load a different dodot. — the evidence line names both versions. Your shells resolve `dodot` to a different install than the one you just ran — typically a stale binary earlier on `$PATH`. `dodot probe shell-init --trace-hook` names the rc file, the hook line, and the exact binary your shell finds there, plus every candidate it passed over (dangling symlinks included).

        For a measured answer rather than an inferred one — including the case where the hook is in your rc but something earlier in the file fails before reaching it — run bare `dodot probe shell-init` for a non-writing targeted check. `dodot install --write` performs the same check after wiring the hook, while `dodot up` uses it when existing evidence is inconclusive. `dodot probe shell-init --trace-hook` performs the explicit rc diagnosis ([./commands/probe.lex] §4).
