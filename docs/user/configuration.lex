...
10. The `[shell]` Section
    ...
    A brew that is installed but fails to answer `brew shellenv` — mid-upgrade, say — is treated differently from a missing one: `dodot up` warns you, quoting brew's exit code or error, and keeps the block from the last successful capture so your shells keep working through the hiccup. Only a genuinely absent brew drops the block.
