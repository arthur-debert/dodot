...
6. Global flags
    ...
    The dotfiles root is not a flag. dodot resolves it by checking `$DOTFILES_ROOT` first, then `git rev-parse --show-toplevel`, then the current working directory. See [./glossary/dotfiles-root.md]. The first mutating command on an implicitly discovered root asks you to approve it — see [./safety-lock.md].
