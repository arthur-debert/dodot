...
7. Where Rendered Output Lives
    ...
    `dodot up` will not overwrite a rendered file whose bytes have diverged from the cached baseline (i.e. you've edited the deployed file in place since the last `up`). The render is skipped, the user's edits stay on disk, and a one-line warning surfaces. Resolve via `dodot transform check` (auto-merge the deployed-side edit back into the source) or re-run with `--force` (overwrite). Staleness is defined from file content only — env vars referenced via `{{ env.X }}` are read live at render time and are intentionally not part of the cache-invalidation signal. Plain `dodot up` re-renders templates every run, so an env-var change is picked up automatically as long as the deployed file still matches its baseline; the only case where `--force` is needed is when the divergence guard is preserving an in-place edit on the deployed file. See [./../proposals/preprocessing-pipeline.lex] §6.4.
