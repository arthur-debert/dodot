Handler Registry
    ...
    *Generated from the handler registry — do not edit.* Run `pixi run gen-docs` to regenerate this page from `crates/dodot-lib/src/handlers/catalog.rs`; the test suite fails when it drifts. Every other document links here rather than restating the tables.
...
1. Handlers
    ...
    Registered handlers:
        ...
        | `homebrew` | `Provision`  | `CodeExecution` | Precise    | Exclusive | `Brewfile`                                                                                                                                                                       | Runs `brew bundle` once per content hash                                         |
        | `nix`      | `Provision`  | `CodeExecution` | Precise    | Exclusive | `packages.nix`                                                                                                                                                                   | Runs `nix profile install` once per content hash                                 |
        | `install`  | `Setup`      | `CodeExecution` | Precise    | Exclusive | `install.sh`, `install.bash`, `install.zsh`                                                                                                                                      | Runs the script once per content hash                                            |
