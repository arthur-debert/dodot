...
5. Configuration vs Code Execution
    ...
    5.2. Code execution handlers
        ...
        *External.* `externals.toml` entries refresh on their own, because there is no user-authored code to re-run — only content to keep current. The sentinel payload is the entry's declared signature: the sha256 you wrote down for a `file` entry, the upstream HEAD returned by a cheap `git ls-remote` for a `git-repo`. When the signature no longer matches what was recorded, the executor re-fetches without asking. Bumping the sha256, or the upstream moving, is the trigger — not `--provision-rerun`.
