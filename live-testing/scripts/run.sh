...
if [ $# -gt 0 ]; then
    ...
    docker compose -f "$SCRIPT_DIR/../containers/docker-compose.yml" run --rm dodot-dev /bin/bash -c "cd /workspace && $SCRIPT_PATH $*"
else
    ...
    docker compose -f "$SCRIPT_DIR/../containers/docker-compose.yml" run --rm dodot-dev
fi