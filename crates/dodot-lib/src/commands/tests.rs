...
use std::sync::Arc;
    ...
    fn status_shows_pending_before_up() {
        ...
        for file in &result.packs[0].files {
            assert_eq!(file.status, "pending", "file {} should be pending", file.name);
        }
    }
    ...
    fn up_generates_shell_init() {
        ...
        let init_content = env.fs.read_to_string(&env.paths.init_script_path()).unwrap();
        assert!(init_content.contains("aliases.sh"), "init script: {init_content}");
    }
    ...
    fn down_removes_deployed_state() {
        ...
        let status = commands::status::status(None, &ctx).unwrap();
        for file in &status.packs[0].files {
            assert_eq!(file.status, "pending", "file {} should be pending after down", file.name);
        }
    }
    ...
    fn adopt_moves_file_and_creates_symlink() {
        ...
        let result =
            commands::adopt::adopt("vim", std::slice::from_ref(&source), false, &ctx).unwrap();
