...
mod overlay_errors_tests {
    ...
    fn failed_run(pack: &str, relative_path: &str, message: &str) -> OperationResult {
        OperationResult::fail(
            Operation::RunCommand {
                pack: pack.into(),
                handler: "install".into(),
                executable: "bash".into(),
                arguments: vec!["--".into(), format!("/packs/{pack}/{relative_path}")],
                sentinel: "install.sh-0000000000000000".into(),
    ...
    /// Status rows are keyed by pack-relative path. A run-once
    /// operation carries the same path, so two files sharing a
    /// basename each flip their own row. Matching on the basename
    /// instead would land both failures on whichever row came first,
    /// and the second would overwrite the first's note.
    #[test]
    fn two_run_once_files_sharing_a_basename_flip_their_own_rows() {
        let packs = vec![DisplayPack::new(
            "tools".into(),
            vec![
                row("install.sh", "install"),
                row("extras/install.sh", "install"),
            ],
        )];
        let pack_results = vec![PackResult {
            ...
            success: false,
            operations: vec![
                failed_run("tools", "install.sh", "top-level blew up"),
                failed_run("tools", "extras/install.sh", "nested blew up"),
            ],
            error: None,
        }];
        let mut notes = Vec::new();

        let out = overlay_errors(
            packs,
            &pack_results,
        ...
        assert_eq!(out[0].files.len(), 2, "no row was synthesized");
        let note_of = |name: &str| {
            let f = out[0].files.iter().find(|f| f.name == name).unwrap();
            assert_eq!(f.status, "error", "row `{name}` must be the failure");
            notes[(f.note_ref.expect("row needs a note") - 1) as usize]
                .body
                .clone()
        };
        assert_eq!(note_of("install.sh"), "top-level blew up");
        assert_eq!(note_of("extras/install.sh"), "nested blew up");
    }

    /// The other half of the same rule: a failure must never be
    /// attached to a row it only shares a basename with. `install.sh`
    /// failing does not make `extras/install.sh` the culprit — with no
    /// row of its own to flip, the failure gets a row of its own.
    #[test]
    fn a_failure_does_not_flip_a_row_it_only_shares_a_basename_with() {
        let packs = vec![DisplayPack::new(
            "tools".into(),
            vec![row("extras/install.sh", "install")],
        )];
        let pack_results = vec![PackResult {
            pack_name: "tools".into(),
            success: false,
            operations: vec![failed_run("tools", "install.sh", "top-level blew up")],
            error: None,
        }];
        let mut notes = Vec::new();
