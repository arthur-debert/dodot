...
pub fn generate_init_script(fs: &dyn Fs, paths: &dyn Pather) -> Result<String> {
    ...
    writeln!(
        script,
        "# Generated by dodot — do not edit manually."
    )
    .unwrap();
    writeln!(
        script,
        "# Regenerated on every `dodot up` / `dodot down`."
    )
    .unwrap();
...
mod tests {
    ...
    fn multiple_packs_combined() {
        ...
        assert_eq!(source_count, 2, "expected 2 source lines, script:\n{script}");
    }
    ...
    fn write_init_script_creates_executable_file() {
        ...
        let script_path =
            write_init_script(env.fs.as_ref(), env.paths.as_ref()).unwrap();
