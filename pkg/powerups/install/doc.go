...
// # Overview
//
// The InstallScriptPowerUp executes shell scripts for one-time setup tasks that should
...
// 4. **PowerUp Invocation**: Matched files are passed to InstallScriptPowerUp.Process()
//
...
// # Options
//
// Currently, InstallScriptPowerUp accepts no configuration options.
...
// 3. InstallScriptPowerUp checks sentinel file
...
// - **BrewPowerUp**: For Homebrew packages (also install phase)
// - **InstallScriptPowerUp**: For custom setup logic (install phase)
// - **ProfilePowerUp**: For shell configuration (deploy phase)
//
// Use InstallScriptPowerUp when you need custom logic that package managers
