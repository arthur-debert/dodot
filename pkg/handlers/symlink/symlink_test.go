...
func TestSymlinkHandler_ProcessMatches(t *testing.T) {
	homeDir := t.TempDir()
	...
	assert.Equal(t, SymlinkHandlerName, actions[0].HandlerName)
	assert.Equal(t, config.Default().Priorities.Handlers["symlink"], actions[0].Priority)
...
func TestSymlinkHandler_PreservesDirectoryStructure(t *testing.T) {
	homeDir := t.TempDir()
	oldHome := os.Getenv("HOME")
