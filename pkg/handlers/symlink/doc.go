...
// # Overview
//
// The SymlinkHandler is one of dodot's core handlers that creates symbolic links from
...
// 4. **Handler Invocation**: Matched files are passed to SymlinkHandler.Process()
//
...
//
// The SymlinkHandler's deployments are tracked via the `DODOT_SYMLINKS` environment
...
// # Options
//
// The SymlinkHandler accepts one option:
//
...
//
// - **InstallScriptHandler**: Runs once during install, for setup tasks
// - **BrewHandler**: Installs packages, runs during install only
// - **ProfileHandler**: Modifies shell profiles, runs during deploy
// - **SymlinkHandler**: Creates config symlinks, runs during deploy
//
// The SymlinkHandler is the workhorse of dodot - most users primarily use it
