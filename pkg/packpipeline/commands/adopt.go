...
func (c *AdoptCommand) ExecuteForPack(pack types.Pack, opts packpipeline.Options) (*packpipeline.PackResult, error) {
	logger := logging.GetLogger("packpipeline.adopt")
	logger.Debug().
		Str("pack", pack.Name).
	...
	for _, fileName := range adoptedFiles {
		statusResult.Files = append(statusResult.Files, packcommands.FileStatus{
			Path:   fileName,
			Status: packcommands.Status{State: packcommands.StatusStateSuccess},
		})
	}
