...
func (c *InitCommand) ExecuteForPack(pack types.Pack, opts packpipeline.Options) (*packpipeline.PackResult, error) {
...
func InitPreprocess(packName string, dotfilesRoot string, fs types.FS) error {
	logger := logging.GetLogger("packpipeline.init.preprocess")
	logger.Debug().
		Str("pack", packName).
