...
func TestDeployPacks_InvalidPack(t *testing.T) {
	// Create test environment
	tempDir := testutil.TempDir(t, "deploy-invalid")
	dotfilesDir := filepath.Join(tempDir, "dotfiles")
...
func TestDeployPacks_EmptyPack(t *testing.T) {
	// Create test environment
	tempDir := testutil.TempDir(t, "deploy-empty")
	dotfilesDir := filepath.Join(tempDir, "dotfiles")
	homeDir := filepath.Join(tempDir, "home")
	...
	// Create empty pack
	testutil.CreateDir(t, dotfilesDir, "empty")

	// Deploy empty pack
	ctx, err := DeployPacks(DeployPacksOptions{
		DotfilesRoot:       dotfilesDir,
		PackNames:          []string{"empty"},
		DryRun:             false,
		EnableHomeSymlinks: true,
	})
	...
	// Empty packs with no actions might not appear in results
	// This is expected behavior - if nothing matches, nothing gets processed
	testutil.AssertEqual(t, "deploy", ctx.Command)
	testutil.AssertFalse(t, ctx.DryRun, "Should not be dry run")
}
