...
- **Nix handler — first slice (#161, PR 1).** `packages.nix` at pack root is now matched and routed to a new `nix` handler, which runs `nix profile install --file <path>` once per content hash via the same `RunOnceHandler` machinery as `install` / `homebrew`. Inherits the three-state notify-don't-rerun policy and `dodot status` integration unchanged. Pre-flight shape validation invokes `nix eval --apply` to classify the manifest; v1 accepts list-of-derivations and bare-derivation forms (e.g. `{ pkgs ? import <nixpkgs> {} }: with pkgs; [ ripgrep fd ]`) and rejects attribute-set manifests with a targeted error pointing at the list form. Default mapping: `nix = "packages.nix"`. User-facing handler reference page lands in PR 2 of #161.
